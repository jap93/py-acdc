...
class Field:
    ...
    def __init__(self):
        ...
        self.fld = rigidion.RigidIon("potentials")
    ...
    def calculate_forces(self, pos_r: np.ndarray, force: np.ndarray, stress: np.ndarray, lat_vector: np.ndarray, rcp_vector: np.ndarray, 
        ...
        #                    int numAtoms)
        new_data = self.fld.calculateForces(pos_r, stress, lat_vector, rcp_vector, charge, atm_label, frozen, natoms)
        
        total_energy.rcpEnergy = new_data[natoms][0]
        total_energy.realEnergy = new_data[natoms][1]
        total_energy.vdwEnergy = new_data[natoms][2]
